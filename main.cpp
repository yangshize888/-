...
struct favorites
{
    char english[20];
    char chinese[30];
    struct favorites * next;//用于存储收藏单词表的链表
};
...
int count_favorites(struct favorites * head)//收藏单词表计数函数
{
    int count=0;
    struct favorites * p;
    p=head->next;
...
void print_word(struct favorites * head)//将收藏单词表输出到屏幕上
{
    struct favorites * p;
    p=head->next;
    while(p!=NULL)
...
struct favorites * luru_favorites()//从文件中读取收藏单词表
{
    int i,count;
    struct favorites *head,*p,*q;
    char cunchu[30],ch;
    FILE * fp;
    p=(struct favorites *)malloc(sizeof(struct favorites));
    p->next=NULL;
    head=p;//分配一个不储存数据的头结点，但是p在下面要参与循环，会发生变化，故使用一个head指针确定p的位置，head的位置不改变，并作为返回值
    fp=fopen("favorites.txt","r");
    ch=fgetc(fp);
    if(ch==EOF)
    {
    ...
    if(fp==NULL)
    {
        printf("未找到favorites.txt");
        exit(0);
    }
    for(i=0;i<k;i++)
    {
        q=(struct favorites *)malloc(sizeof(struct favorites));
        fscanf(fp,"%s",cunchu);
...
int luru(struct danci word[])//用于录入文件中的词表
{
    int i,j,count=0;
    FILE * fp;
    fp=fopen("C:\\Users\\Lenovo\\OneDrive\\Desktop\\github_project\\back_code\\vocabulary.txt","r");
    if(fp==NULL)
    {
        printf("找不到vocabulary.txt");
        exit(0);
    }
    for(i=0;i<k;i++)
    {
...
int delete_words(int n2,struct favorites * head)
{
    int panduan,flag=0;
    struct favorites *p,*q,*s;
    char a[30];
    FILE * fp;
    print_word(head);
    if(n2==0)
    ...
    fclose(fp);
    printf("文件已更新，现在有%d个收藏单词\n",count_favorites(head));
    print_word(head);
    return count_favorites(head);
}
int find_word(int a,struct danci word[])
{
    int moshi,n;
    char b[30];
    printf("1代表英译汉，2代表汉译英\n");
...
int add_favorites(int n2,int a,struct favorites * head,struct danci word[])
{
    int cishu,flag,count=0,panduan,pin=n2;
    FILE * fp;
    struct favorites *p,*q,*start,*i;
    printf("请输入你要添加单词的个数：");
    scanf("%d",&cishu);
    q=(struct favorites *)malloc(sizeof(struct favorites));
    q->next=NULL;
    ...
    count=count_favorites(head);
    printf("收藏单词目前有%d个\n",count);
    print_word(head);
    return count;
}
void add_words()//运行添加函数时须关闭words.txt，否则将无法写入文件
{
    int i=1,flag;
...
void judge_favorites(int n2,struct favorites * head)
{
    int flag1,flag2,cishu,tixing,input;
    //tixing用来判断输出正确还是错误,input用于储存用户输入
    struct favorites *p1,*p2,*q;
    char shuru[30];
    if(n2==0)
    {
        printf("无收藏单词");
...
int main() {
    ...
    if (choice == 1) {
        ...
        std::cout << "注册成功，正在尝试自动登录...\n";
    }

    uid = User::loginUser(username, password);
    if (uid == -1) {
        std::cout << "登录失败！\n";
        std::this_thread::sleep_for(std::chrono::seconds(5));
        return 0;
    }
    std::cout << "登录成功，用户ID: " << uid << "\n";

    // 可选：显示错题复习计划
    Mistake::showReviewPlan(uid);

    // 主逻辑：词库初始化与菜单功能
    int n1, n2, judge;
    char panduan[10] = "yes";
    struct favorites* head;

    n1 = luru(word);  // 加载单词
    head = luru_favorites();
    n2 = count_favorites(head);
    printf("录入了%d个收藏单词\n\n", n2);
    ...
    while (strcmp(panduan, "yes") == 0) {
        ...
        switch (judge) {
            ...
            case 3:
                system("cls");
                tiankong(n1, word);
                Mistake::addMistake(uid, "示例单词", "2025-05-14");
                break;
            case 4:
                system("cls");
                add_words();
            ...
            case 7:
                system("cls");
                n2 = add_favorites(n2, n1, head, word);
                break;
            case 8:
                system("cls");
                n2 = delete_words(n2, head);
                break;
            case 9:
                system("cls");
                chachong(n1, word);
            ...
            case 10:
                system("cls");
                judge_favorites(count_favorites(head), head);
                break;
        }
        printf("输入yes继续：");
        scanf("%s", panduan);
